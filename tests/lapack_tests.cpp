...
int main(int argc, char**){
    ...
    verbose = (argc > 1);
    std::string name = "HALA LAPACK TESTS";
    begin_report(name);
    ...
    for(int gpuid=0; gpuid<hala::gpu_device_count(); gpuid++){
        begin_report(std::string("Testing GPU LAPACK: device ") + std::to_string(gpuid) + " (with engine)");
        ...
        begin_report(std::string("Testing GPU LAPACK: device ") + std::to_string(gpuid) + " (no engine)");
